...
def get_player_dirs(root_dir: Path):
    """Get all player directories in data/atp"""
    if not root_dir.exists():
        print(f"Error: Data directory not found at {root_dir}")
        return []
    
    return [d for d in root_dir.iterdir() if d.is_dir() and not d.name.startswith('.')]
...
def scrape_player(player_dir, headers, timeout):
...
def main():
    ...
    gs_cfg = config.get("atp_grandslam", {})
    
    # Parameters
    root_str = gs_cfg.get("root", DEFAULT_DATA_DIR_REL)
    data_dir = (script_dir / root_str).resolve()
    delay = gs_cfg.get("delay", 1.0)
    timeout = common_cfg.get("timeout", 45)
    
    user_agent = gs_cfg.get("user_agent", DEFAULT_USER_AGENT)
    headers = {'User-Agent': user_agent}

    player_dirs = get_player_dirs(data_dir)
    print(f"Found {len(player_dirs)} player directories in {data_dir}")
    ...
    for p_dir in player_dirs:
        if scrape_player(p_dir, headers, timeout):
            success += 1
        count += 1
