...
export class DataService {
  ...
  async loadAll(onProgress = () => {}) {
    ...
    onProgress({
      ...
      maxYear,
      liveYearSync,
      message: 'Historic WTA data loaded'
    });
  }
  ...
  async #fetchCsvSource(url, sourceLabel, sourceType) {
    ...
    if (!this.#looksLikeMatchCsv(csvText)) {
      throw new Error('response did not look like tour match CSV data');
    }
  ...
  getRecords(category = 'all') {
    ...
    rows.push(row('titles', 'Most WTA 1000 Titles', `${mastersTitles.max}`, mastersTitles.holders, '4, 2, 1'));

    const atp500Titles = top(players, (p) => p.atp500Titles);
    rows.push(row('titles', 'Most WTA 500 Titles', `${atp500Titles.max}`, atp500Titles.holders, '2, 1'));

    const atp250Titles = top(players, (p) => p.atp250Titles);
    rows.push(row('titles', 'Most WTA 250 Titles', `${atp250Titles.max}`, atp250Titles.holders, '2, 1'));
